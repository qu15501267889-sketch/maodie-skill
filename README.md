...
### 🔍 模式识别（v2.0+）
- 自动识别反复出现的思维模式（如"越得不到越想追"、"找理由"倾向）
- 识别常见情绪触发点和应对倾向
- 在合适的时候自然提醒用户
...
```
maodie/                      ← 耄耋主技能
├── SKILL.md                 ← 核心人格、心理分析框架、v2.2.0 哲学
...
|------|------|
| v1.0.0 | 初始发布 |
| v2.0.0 | 记忆系统 + 模式识别 + 复盘模式 |
| v2.1.1 | 修复自动存档 BUG |
| v2.1.2 | 修复隐私上传问题，添加 .gitignore |
| v2.1.6 | 每5次回答静默写入 |
| v2.1.7 | 每次回答写入一行（修复计数BUG） |
| v2.1.8 | 降低记录标准，只写重大事件 |
| v2.1.93 | 新增回答前先判断再写入机制 |
| **v2.2.0** | **核心哲学调整：不迎合用户，基于事实和判断直接说结论** |
| v2.2.1 | 🐱🐱记下了格式：括号列出写入文件 + memory/多文件自动更新 |
